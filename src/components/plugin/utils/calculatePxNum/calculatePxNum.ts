...
export default function calculatePxNum(value: number, minValue: number, maxValue: number, elementSize: number): number {
  ...
  return (Math.abs((value as number) - minValue) / Math.abs(maxValue - minValue)) * elementSize - PIN_SIZE / 2;
}
